...
class Page(HTMLParser):
    def __init__(self): super().__init__();self.sides=[];self.players=0
    def handle_starttag(self,tag,attrs):
        a=dict(attrs)
        if 'data-side' in a:self.sides.append(a['data-side'])
        if tag=='button' and ('data-audio' in a or a.get('data-control')=='play'):self.players+=1
...
with tempfile.TemporaryDirectory(prefix='ccpt-validation-') as temp:
    ...
    for cid in col.find_cards(''):
        card=col.get_card(cid);note=card.note();q=card.question();page=Page();page.feed(q)
        assert page.sides==['read'] and page.players==1,'Single complete page and exactly one narration control required'
        ...
        if 'data-audio-pending="1"' in q:
            assert a.allow_text_only_test, 'Audio pending: not a completed audio package'
            pending+=1
        for name in set(re.findall(r'data-audio="([^"]+)"',note['BackHTML'])):
