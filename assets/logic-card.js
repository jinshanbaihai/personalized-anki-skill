(function(){
 ...
 function drawMap(){root.querySelectorAll('.mindmap').forEach(map=>{const svg=map.querySelector('.map-lines'),r=map.getBoundingClientRect();svg.setAttribute('viewBox',`0 0 ${r.width} ${r.height}`);svg.innerHTML='';const narrow=false;function connect(a,b,level){const x=a.getBoundingClientRect(),y=b.getBoundingClientRect();let d;if(narrow){const sx=(level===0?r.left+10:x.left-12)-r.left,sy=(level===0?x.bottom:x.top+x.height/2)-r.top,ex=y.left-r.left,ey=y.top+y.height/2-r.top;d=`M ${sx} ${sy} V ${ey} H ${ex}`;}else{const sx=x.right-r.left,sy=x.top+x.height/2-r.top,ex=y.left-r.left,ey=y.top+y.height/2-r.top,mx=sx+(ex-sx)*.5;d=`M ${sx} ${sy} H ${mx} V ${ey} H ${ex}`;}const path=document.createElementNS('http://www.w3.org/2000/svg','path');path.setAttribute('d',d);svg.appendChild(path);}const top=map.querySelector('.map-root');map.querySelectorAll('.map > .branch').forEach(branch=>{const head=branch.querySelector('.branch-head');connect(top,head,0);branch.querySelectorAll(':scope > .leaves > .leaf').forEach(leaf=>connect(head,leaf,1));});});}
